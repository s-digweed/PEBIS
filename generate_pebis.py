...
# Tried in order - first one that returns valid data wins.
SOURCE_URLS = [
    "https://i.mjh.nz/PBS/.app.json.gz",
    "https://i.mjh.nz/PBS/app.json.gz",
    "https://i.mjh.nz/PBS/.app.json",
]
...
def try_url(url: str) -> dict:
    """Fetch and parse one candidate URL. Raises on any failure."""
...
def fetch_source() -> tuple[dict, bool]:
    ...
    for url in SOURCE_URLS:
        ...
        try:
            data = try_url(url)
            print(f"  OK - {len(data['channels'])} stations")
            return data, True
        except (requests.RequestException, ValueError, json.JSONDecodeError) as exc:
            print(f"  failed: {exc}")

    print("WARNING: all upstream URLs failed")
    backup = load_backup()
    if backup is None:
